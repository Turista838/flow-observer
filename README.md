...
- Annotate a **public** `StateFlow` or `SharedFlow` only — not a private `MutableStateFlow` / `MutableSharedFlow` backing property.
- If `tag` is omitted, it defaults to `ViewModelName.propertyName`.
- `attachFlowObserver()` is **generated by KSP**. After adding `@ObserveFlow`, build the module (Sync / Make Project) so the function is created; then `init { attachFlowObserver() }` will resolve.
